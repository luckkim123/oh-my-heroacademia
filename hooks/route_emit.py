...
def build_routing_context(cards_dir: Path) -> str:
    ...
    return (
        "<omha-routing>\n"
        "매 턴, 응답 맨 앞에 ROUTE 한 줄을 무조건 낸다(handle-directly 여도 낸다 — ROUTE\n"
        "생략은 없다). 3+ 액션/복수파일/모호한 요청이면 ROUTE 위에 ANALYZE 블록을 더\n"
        "얹는다 — '3+ 액션'은 *ANALYZE 를 추가할* 조건이지 *ROUTE 를 낼* 조건이 아니다.\n"
        "아래 하네스 카드로 어느 레인인지 판정하라. 레인만 정하라 — 레인 안 스킬\n"
        "콕집기는 해당 plugin 이 한다.\n\n"
        "ROUTE 는 채울 출력 슬롯이 아니라 *매 턴 새로 내리는 판정*이다(출력은 매 턴\n"
        "강제지만 판정도 매 턴이다 — 둘을 분리하지 말 것). 직전 턴 ROUTE 를 관성으로\n"
        "복사하지 말고 *이번 요청* 기준으로 처음부터 다시 판정하라. 핵심 함정:\n"
        ...
        "닫는 재확인(턴 종료 전, 위치는 그대로 — ROUTE 는 맨 앞 유지): ROUTE 를 맨\n"
        "앞에 찍는 건 *행동 전 commitment 게이트*다(그 레인 안에서 이번 턴을 한다는\n"
        "선언). 그래서 위치는 끝으로 옮기지 않는다 — 대신 본문을 다 쓴 *뒤* 한 번\n"
        "대조하라: 내가 실제로 한 작업이 맨 앞 ROUTE 와 같은 레인이었나? 깊이\n"
        "생각해보니(또는 본문 도중 무거운 하위작업·산출물 수정으로) 레인이 달라졌다면,\n"
        "그 사실을 한 줄로 명시하고 *갱신된 ROUTE 를 다시 찍어라*. 맨 앞 판정이\n"
        "사고보다 먼저라 사고가 판정을 못 고치는 함정은 이 닫는 대조로 막는다. (레인이\n"
        "안 바뀐 단순 턴이면 갱신 불필요 — 과흡인 금지, 바뀐 경우에만 재발행.)\n\n"
