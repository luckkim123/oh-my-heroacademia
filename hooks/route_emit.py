...
def build_routing_context(cards_dir: Path) -> str:
    ...
    return (
        ...
        "단, 결론이 가치·타당성 판단을 settled 로 단정하는 것이고 되돌리기 비싼 downstream\n"
        "행동을 유발하면 handle-directly 가 아니다 — 아래 '설계·타당성 판단 단정 직전'\n"
        "게이트로 재라우팅(논증의 self-approval 회피).\n\n"
        ...
        "· 3순위 — 그것도 없음 → handle-directly(직접 수행). 단, 답이 '설계가 옳은가/\n"
        "  학술·물리적으로 타당한가'류 판단을 settled 로 단정하는 것이면 아래 '설계·타당성\n"
        "  판단 단정 직전' 게이트를 적용하라. handle-directly 는 사실 전달·요약·잠정 의견에\n"
        "  한한다.\n\n"
