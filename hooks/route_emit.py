...
def build_routing_context(cards_dir: Path) -> str:
    ...
    return (
        "<omha-routing>\n"
        "ROUTE 는 출력 슬롯이 아니라 *매 턴 새로 내리는 판정*이다. 판정은 매 턴 하되,\n"
        "출력은 *직전 턴과 레인이 바뀌었을 때만* 낸다(첫 턴은 항상). 같은 레인이면\n"
        "ROUTE 줄만 생략하는 것이지 판정을 건너뛰는 게 아니다 — 직전 ROUTE 를 관성으로\n"
        "복사하지 말고 *이번 요청* 기준으로 처음부터 다시 판정하라. 레인만 정하라 —\n"
        ...
        "  전체 트리를 먼저 확인하라.\n"
        "· 예외: 진짜 3-4줄짜리 단일 파일·단일 사실 lookup 은 직접(과흡인 금지). 여러\n"
        "  파일을 읽거나 코드 동작을 해석하거나 구조를 파악해야 하면 lookup 이 아니라\n"
        "  조사이므로 위임한다. citation-bound 논문 자료조사는 OMC 병렬 금지.\n\n"
        ...
        "사용자에게 확인하라.\n\n"
        "출력 순서 (ROUTE 를 낼 때 = 레인 전환 턴 또는 첫 턴): 응답 맨 처음에, 게이트\n"
        ...
        "이면 ANALYZE 없이 ROUTE 만 맨 앞 줄에. 단 직전과 레인이 같으면 ROUTE 줄 자체를\n"
        "생략한다(ANALYZE 는 게이트 해당 시 레인 변화와 무관하게 낸다 — 요구사항 분해는\n"
        "출력 노이즈가 아니라 작업 정확도용).\n"
        ...
        "찍어라*(레인이 바뀌었으니 출력 생략 규칙에 안 걸린다). 직전과 그대로면 재발행\n"
        "불필요(과흡인 금지) — 판정만 조용히 확인하고 넘어간다.\n"
        "</omha-routing>"
    )
