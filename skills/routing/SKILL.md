...
- **3순위** — handle-directly(직접 수행). 소거법으로는 도달하지 않는다:
  *적극적 정의* 셋 중 하나일 때만 성립한다 — 단일 사실 lookup, 이미 정해진 것의
  요약·설명, 가벼운 대화 응답·잠정 의견(목표가 이미 정해진 경우). **셋 중 어디에도
  못 넣으면 handle-directly 가 아니다 — 2순위 작업방식으로 내려라**(도메인이 없으면
  oh-my-claudecode). '아무것도 안 걸렸다'는 소거법은 근거가 못 되고, '스킬·에이전트를
  안 썼다'는 결과지 근거가 아니다 — 위임 여부로 판정하지 마라. 목표
  미결정(2.5순위)이거나 여러 턴짜리 설계 탐색이면 아니다. 답이 '설계가 옳은가/학술·물리적으로 타당한가'를 settled 로 단정하는
